 ...
 */
TSL_tKCoeff_T TSL_ecs_CalcK(TSL_ObjectGroup_T *objgrp, TSL_tKCoeff_T k_slow,
		TSL_tKCoeff_T k_fast) {
	...
	for (idx_obj = 0; idx_obj < objgrp->NbObjects; idx_obj++) {
		...
		if ((THIS_OBJ_TYPE == TSL_OBJ_TOUCHKEY)
				|| (THIS_OBJ_TYPE == TSL_OBJ_TOUCHKEYB)) {
...
TSL_Status_enum_T TSL_ecs_Process(TSL_ObjectGroup_T *objgrp) {
	...
	if ((objgrp->StateMask & TSL_STATE_RELEASE_BIT_MASK)
			&& !(objgrp->StateMask & TSL_STATEMASK_ACTIVE)) {
	...
	if (objgrp->ECS_wait && (!objgrp->ECS_exec)) {
		...
		if (TSL_tim_CheckDelay_ms(TSLPRM_ECS_DELAY, &objgrp->ECS_start_time)
				== TSL_STATUS_OK) {
			objgrp->ECS_exec = 1;
		}
	}
